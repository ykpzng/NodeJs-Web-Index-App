...
export default class SiteIndexleme extends Component {
  ...
  getData = (url0) => {
    axios.post(`http://localhost:8000/semantik`, { url0 })
      .then(res => {
        const data = res.data;
  ...
  getDataGroup = async (url) => {
    for (let i = 0; i < url.length; i++) {
      let urlgroup = url[i];
      await axios.post(`http://localhost:8000/semantik`, { urlgroup })
        .then(res => {
          const data = res.data;
  ...
  render() {
    ...
    return (
      <div className="components">
        <p>Girilen bir URL ile URL grubunun benzer kelimelerini arasındaki benzerlik skorlamasını verir...</p>
        <br />
        ...
        <br />
        <Button fluid color='teal' >Semantik Analiz Yap</Button>
        <br />
        {this.state.open && (
          <div>
            <Table celled>
